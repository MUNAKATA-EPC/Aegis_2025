...
float get_max_value_in_array(float* ar)//モーターの数列の中のどの値が一番高いか
{
  float ret;
...
void Move_Deg(int deg, float power)
{
  ...
  int pid_val = PID_gain(1.1, 0.5, 0.7, 0, gyro_data, atack_goal_dir *-1, 40); // int pid_val = PID_gain(0.7,0.3, 0.5, 9.5, gyro_deg, goaldir, 40);
  // pidsuutikawarisugi

  power_[0] = -sin((deg - 45) / 180.0 * 3.14) * power;  //+ pidval
  power_[1] = sin((deg + 45) / 180.0 * 3.14) * power;   //- pidval
  power_[2] = -sin((deg - 135) / 180.0 * 3.14) * power; //+ pidval
  power_[3] = sin((deg + 135) / 180.0 * 3.14) * power;  //- pidval
