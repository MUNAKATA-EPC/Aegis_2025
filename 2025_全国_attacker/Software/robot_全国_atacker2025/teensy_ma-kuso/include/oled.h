...
void Line_cal(int line)
{
  line_det = line;
}
void Camera_dir_cal(int goal_dir, int goal_dis, int cout_green)
{
  yellow_x = sin(radians(goal_dir)) * 20;
  yellow_y = cos(radians(goal_dir)) * 20;

  yellow_screen = goal_dir;

  blue_screen = goal_dis;
...
void oled_button()
{
  ...
  right_total = digitalRead(left_button);
  left_total = digitalRead(right_button);
